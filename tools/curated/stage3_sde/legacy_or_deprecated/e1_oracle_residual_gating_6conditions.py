...
RECON_DIR = PROJECT_ROOT / "outputs" / "stage4" / "reconstructed_stage3_cond_outputs"
...
OUT_DIR = PROJECT_ROOT / "outputs" / "stage4" / "e1_oracle_residual_gating_6conditions"
...
PROTOCOL_VALIDATION_PATH = OUT_DIR / "e1_protocol_validation_summary.csv"
...
COND_OUTPUT_PATHS = {
    ...
    "jump_medium": RECON_DIR / "jump_cond_residual_t20_refined_reconstructed.npy",
    "burst_medium": CACHE_DIR / "burst_cond_residual_t20_refined.npy",
    "bias_medium": CACHE_DIR / "bias_cond_residual_t20_refined.npy",
    "combined_medium": RECON_DIR / "combined_cond_residual_t20_refined_reconstructed.npy",
}

COND_SOURCE_TYPES = {
    "gaussian_medium": "archived_original",
    "drift_medium": "archived_original",
    "burst_medium": "archived_original",
    "bias_medium": "archived_original",
    "jump_medium": "matched_protocol_recomputed",
        ...
        inventory.append(
            {
                ...
                "conditional_source": COND_SOURCE_TYPES.get(degradation, "unknown"),
            }
        )
...
def build_protocol_validation_summary(
    ...
    inventory_by_deg = {row["degradation"]: row for row in inventory}
    rows = []
    for degradation in DEGRADATION_ORDER:
        arrays = loaded[degradation]
        clean = arrays["clean"]
        cond = arrays["cond"]
        degraded = arrays["degraded"]
        cond_ade = frame_error(cond, clean).mean(axis=1)
        noisy_ade = frame_error(degraded, clean).mean(axis=1)
        stage3_cond_mean, stage3_cond_std = get_stage3_ade(metrics_df, degradation, "cond_residual_t20")
        stage3_noisy_mean, _ = get_stage3_ade(metrics_df, degradation, "noisy_input")
        rows.append(
            {
                "degradation": degradation,
                "conditional_source": COND_SOURCE_TYPES[degradation],
                "conditional_path": inventory_by_deg[degradation]["cond_path"],
                "degraded_path": inventory_by_deg[degradation]["degraded_path"],
                "shape_clean": str(tuple(clean.shape)),
                "shape_degraded": str(tuple(degraded.shape)),
                "shape_cond": str(tuple(cond.shape)),
                "computed_cond_ADE_mean": float(cond_ade.mean()),
                "computed_cond_ADE_std": float(cond_ade.std()),
                "stage3_summary_cond_ADE_mean": stage3_cond_mean,
                "stage3_summary_cond_ADE_std": stage3_cond_std,
                "cond_ADE_mean_abs_diff": abs(float(cond_ade.mean()) - stage3_cond_mean),
                "computed_noisy_ADE_mean": float(noisy_ade.mean()),
                "stage3_summary_noisy_ADE_mean": stage3_noisy_mean,
                "noisy_ADE_mean_abs_diff": abs(float(noisy_ade.mean()) - stage3_noisy_mean),
                "protocol_validation_pass": bool(abs(float(cond_ade.mean()) - stage3_cond_mean) < 1e-6),
            }
        )
...
def build_evaluation_tables(
    ...
    for degradation, arrays in loaded.items():
        ...
        residual_den = float(np.linalg.norm(cond - degraded, axis=-1).mean())
        residual_num = float(np.linalg.norm(e1 - degraded, axis=-1).mean())
        e1_residual_usage_ratio = residual_num / residual_den if residual_den > 0.0 else math.nan
        ...
        for method, pred in methods.items():
            ...
            mid_ade = float(errors[method][mid].mean()) if np.any(mid) else math.nan
            ...
            full_rows.append(
                {
                    ...
                    "ADE_high_confidence": high_ade,
                    "ADE_mid_confidence": mid_ade,
                    "ADE_low_confidence": low_ade,
                    "e1_win_rate_vs_conditional": e1_win_vs_cond,
                    "e1_win_rate_vs_noisy": e1_win_vs_noisy,
                    "residual_usage_ratio": e1_residual_usage_ratio if method == "e1_oracle_gated_residual" else math.nan,
                }
            )
...
def plot_case(eval_cache: dict[str, dict], degradation: str, idx: int, title: str, path: Path) -> None:
    ...
    axe.plot(t, errors["noisy_input"][idx], "C1-o", ms=3, lw=1.1, label="noisy")
    axe.plot(t, errors["stage3_cond_residual_t20"][idx], "C3-o", ms=3, lw=1.1, label="cond")
    axe.plot(t, errors["e1_oracle_gated_residual"][idx], "C0-o", ms=3, lw=1.2, label="E1")
...
def make_figures(eval_cache: dict[str, dict]) -> list[dict[str, str]]:
    ...
    requests = []
    for degradation in DEGRADATION_ORDER:
        if degradation in eval_cache:
            ade = eval_cache[degradation]["ade_traj"]
            idx = int(np.argmax(ade["stage3_cond_residual_t20"] - ade["e1_oracle_gated_residual"]))
            requests.append((f"representative_{degradation}", (degradation, idx), f"Representative {degradation}"))
    requests.extend([
...
def assess_pass_fail(pass_df: pd.DataFrame, missing: list[dict[str, str]]) -> dict[str, object]:
    ...
    attention = ["drift_medium", "burst_medium", "combined_medium"]
    attention_wins = [deg for deg in attention if deg in wins]
    ...
    return {
        ...
        "C1_note": (
            f"E1 beats conditional in {len(wins)}/{len(DEGRADATION_ORDER)} formal conditions; "
            f"attention wins {attention_wins}; missing required per-frame outputs: {missing_required}"
        ),
...
def write_summary(
    ...
    for row in inventory:
        lines.append(
            ...
            f"cond_exists={row['cond_exists']} `{row['cond_path']}`; source={row['conditional_source']}"
        )
    if missing:
            ...
            markdown_table(
                protocol_df[
                    [
                        "degradation",
                        "conditional_source",
                        "computed_cond_ADE_mean",
                        "stage3_summary_cond_ADE_mean",
                        "cond_ADE_mean_abs_diff",
                        "protocol_validation_pass",
                    ]
                ]
            ),
            "",
...
def main() -> None:
    ...
    protocol_df = build_protocol_validation_summary(loaded, metrics_df, inventory)
    ...
    protocol_df.to_csv(PROTOCOL_VALIDATION_PATH, index=False)
    write_summary(
        loaded=loaded,
        missing=missing,
        inventory=inventory,
    ...
    print("STAGE4_E1_ORACLE_GATING_6CONDITIONS_COMPLETE")
    print(f"gaussian ADE reproduction check passed: {reproduction['passed']}")
    print(
        "gaussian cond ADE: "
        f"{float(reproduction['computed_cond_ade_mean']):.9f} "
        f"(stage3 {float(reproduction['stage3_cond_ade_mean']):.9f})"
    )
    print(f"number of degradation conditions evaluated: {len(loaded)}")
    for _, row in protocol_df.iterrows():
        print(f"{row['degradation']}: {row['conditional_source']}, validation={row['protocol_validation_pass']}")
